...
function incrpcorrmat( out, means ) {
	...
	if ( isPositiveInteger( out ) ) {
		...
		throw new TypeError( format( '1JfI8', out ) );
	}
	...
	if ( arguments.length > 1 ) {
		if ( !isVectorLike( means ) ) {
			throw new TypeError( format( '1JfI0', means ) );
		}
	...
	*/
	function accumulator1( v ) {
		...
		if ( !isVectorLike( v ) ) {
			throw new TypeError( format( '1JfI2', v ) );
		}
		if ( v.shape[ 0 ] !== order ) {
			throw new Error( format( '1Jf9g,I6', order, v.shape[ 0 ] ) );
		}
		n = N;
		N += 1;
		r = n / N;
