...
function incrpcorrmat( out, means ) {
	...
	if ( isPositiveInteger( out ) ) {
		...
		throw new TypeError( format( '1JfDf', out ) );
	}
	...
	if ( arguments.length > 1 ) {
		if ( !isVectorLike( means ) ) {
			throw new TypeError( format( '1JfDc', means ) );
		}
	...
	*/
	function accumulator1( v ) {
		...
		if ( !isVectorLike( v ) ) {
			throw new TypeError( format( '1JfDd', v ) );
		}
		if ( v.shape[ 0 ] !== order ) {
			throw new Error( format( '1Jf9g', order, v.shape[ 0 ] ) );
		}
		n = N;
		N += 1;
		r = n / N;
